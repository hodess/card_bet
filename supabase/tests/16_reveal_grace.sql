...
-- 1. l'ouverture de partie n'a pas de sursis : la première carte est déjà là
select ok((select opened_at from auctions where seq = 1) <= now(),
  'enchère 1 : aucun sursis à l''ouverture de la partie');

-- 2. après une adjudication, l'enchère suivante est datée dans le futur
select test_login('00000000-0000-0000-0000-000000000002');
...
select ok((select opened_at from auctions where seq = 2) > now() + interval '2 seconds',
  'enchère 2 : sursis de révélation (~3 s)');
...
-- 3. pendant le sursis, personne n'agit sur l'enchère (p2 mène, p1 est challenger)
select test_login('00000000-0000-0000-0000-000000000001');
